...
void aes_256_enc(uint8_t output[16], const uint8_t key[32], int round, const uint8_t input[16]) {
    int i,j,k;
    uint8_t state[4][4];
    uint8_t w[Nr+1][4][4];
    KeyExpansion(key,w);
...
void g_ctr(uint8_t* X, const uint8_t k[32], uint8_t ICB[16], long len_X, int round, uint8_t* out)
{
	uint8_t CB[16];
	...
	if(len_X > 0)
	{
		...
		for(int i=0;i<len_X;i++)
		{
            ...
            aes_256_enc(encrypt_CB, k, round, CB);
...
void aes256gcm(uint8_t *ciphertext, uint8_t tag[16], uint8_t k[32], const uint8_t IV[12], int round, const uint8_t *plaintext, const unsigned long len_p, const uint8_t* add_data, const unsigned long len_ad) 
{
    ...
    aes_256_enc(en_counter, k, round, J_0);
...
void key_commit_attack_verify(){
    uint8_t K0[32] = {0x2c, 0x72, 0xda, 0x10, 0x52, 0x3f, 0xf5, 0xbc, 0x2d, 0x20, 0xbc, 0x6f, 0xb3, 0x21, 0xfd, 0x0d, 0xde, 0x88, 0x87, 0x04, 0xc9, 0xf7, 0xb0, 0xe7, 0x33, 0xd0, 0x67, 0x66, 0x80, 0xb1, 0x29, 0xb3};
    uint8_t K1[32] = {0x2c, 0x72, 0xda, 0x15, 0x52, 0x3f, 0xf5, 0xbc, 0x2d, 0x20, 0xbc, 0x6a, 0xb3, 0x21, 0xfd, 0x0d, 0xde, 0x88, 0x87, 0x04, 0xc9, 0xf7, 0xb0, 0xe7, 0x33, 0xd0, 0x67, 0x66, 0x80, 0xb1, 0x29, 0xb3};
    uint8_t IV[12] = {0x91, 0xbf, 0x67, 0xe6, 0x96, 0xa9, 0x72, 0x12, 0x73, 0xa9, 0x9a, 0x10};
    uint8_t add_data[16]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01};
    uint8_t plaintext0[48] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0xa0,0x27,0x8d,0x41,0x96,0x6,0xb9,0x3a,0xfa,0x61,0xff,0xfa,0x92,0x9c,0x55,0xe7,0x6d,0x79,0x86,0x1b,0xa7,0xa8,0x53,0x21,0x35,0x6b,0xa0,0x6b,0x33,0x91,0x9a,0x50};
    uint8_t plaintext1[48] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0xf0,0xbb,0x46,0x3e,0x19,0x43,0xff,0x1d,0xeb,0xd5,0xf2,0x23,0x1d,0xe0,0xa4,0x38,0xd6,0x8a,0x9b,0x5,0x6e,0x4d,0x9b,0x50,0x38,0x11,0x93,0x61,0x7c,0xf3,0x2d,0x4d};
    uint8_t ciphertext0[48] = {0};
    uint8_t ciphertext1[48] = {0};
    uint8_t tag0[16] = {0};
    uint8_t tag1[16] = {0};
    aes256gcm(ciphertext0, tag0, K0, IV, 7, plaintext0, 3, add_data, 1);
    aes256gcm(ciphertext1, tag1, K1, IV, 7, plaintext1, 3, add_data, 1);
    printf("IV\n");
    for(int i = 0; i < 12; i++){
        printf("%02x ",IV[i]);
    }printf("\n");
